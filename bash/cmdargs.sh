...
verbose="off"
  case "$1" in
    ...
    echo "-h or --help command to find more information about command"

    ;;
    -v)
    echo "Usage: $(basename $0) [-v]"
    echo "-v can be used after the command for finding verbose for that"
    verbose="on"

    ;;
    -d)
    echo "Consuming the usage: $(basename $0) [-d]"
    echo "-d is used after the command for finding debug for that"
    debug="on"
